 ...
 *   - 6 cuentas demo (admin, organizadora y 4 capitanes) con contraseña Demo2026!
 *   - 1 liga -> 1 temporada -> 2 torneos 1v1 con 4 equipos de 1 persona:
 ...
 *         gran final pendiente — ese es el momento "en vivo" de la demo.
...
function reset() {
  ...
  psql(`
    DELETE FROM torneo WHERE nombre IN ('${TORNEO_A}', '${TORNEO_B}');
    DELETE FROM temporada WHERE liga_id IN (SELECT id FROM liga WHERE nombre = '${LIGA}');
    DELETE FROM liga WHERE nombre = '${LIGA}';
...
async function seed() {
  ...
  console.log('\n== Equipos (1 jugador c/u) ==');
  const equipoIds = {};
  for (const e of EQUIPOS) {
    const equipo = await api('POST', '/teams', {
      token: tokens[e.clave],
      body: { nombre: e.nombre, juegoId: juego.id, descripcion: `Equipo demo capitaneado por ${USUARIOS[e.clave].nombre}.` }
    });
  ...
  async function crearTorneo(nombre, formatoTorneo, horas, premio) {
    const t = await api('POST', '/tournaments', {
      token: tokens.orga,
      body: {
        nombre, juegoId: juego.id, temporadaId: temporada.id, formato: formatoTorneo,
        tamanoEquipo: 1, maxEquipos: 4, fechaInicio: fechaLocal(horas), publico: true,
    ...
    for (const e of EQUIPOS) {
      await api('POST', `/tournaments/${t.id}/inscripciones`, {
        token: tokens[e.clave], body: { equipoId: equipoIds[e.clave], usuarioEnJuego: e.gamertag }
      });
    }
    await api('POST', `/tournaments/${t.id}/iniciar`, { token: tokens.orga });
    log(`${nombre} (id ${t.id}): 4 equipos inscritos, bracket generado.`);
    return t;
  }
  ...
  console.log('\n== Torneo B: en curso, gran final pendiente ==');
  const torneoB = await crearTorneo(TORNEO_B, 'DOBLE_ELIMINACION', 3, 'Trofeo Orbita');
  const abiertas = await resolverHastaDejar(torneoB.id, tokens.orga, 1);
  const granFinal = abiertas[0];
  log(`Gran final pendiente: ${granFinal.equipoANombre} vs ${granFinal.equipoBNombre} (partida ${granFinal.id}).`);
...
------------------------------------------------------------
 ...
 Capitanes     cap1..cap4.demo@brakket.gg (Fenix/Lobos/Cuervos/Titanes)
------------------------------------------------------------
 Torneo A FINALIZADO:  ${TORNEO_A} (id ${torneoA.id})
 Torneo B EN CURSO:    ${TORNEO_B} (id ${torneoB.id}) — gran final pendiente
